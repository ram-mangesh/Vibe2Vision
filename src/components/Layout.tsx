  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="relative z-10 flex">
        ...
        >
          <div className="h-full p-5 m-3 flex flex-col rounded-[22px]"
            style={{
              background: "linear-gradient(160deg, #0f1535 0%, #0d1228 50%, #100d25 100%)",
              border: "1px solid rgba(255, 61, 127, 0.25)",
              boxShadow: "inset -1px 0 0 rgba(255,61,127,0.08), 4px 0 32px rgba(0,0,0,0.6)",
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <div className="text-lg font-bold tracking-wide">AEGIS</div>
                <div className="text-[11px] text-slate-400 -mt-0.5">AI Safety Intelligence</div>
              </div>
            </div>

            {/* Role indicator badge */}
            <div className={`mb-6 px-3 py-2 rounded-xl border text-[11px] font-bold uppercase tracking-wider flex items-center gap-2 ${
              user?.role === "ADMIN"
            ...
            <div className="mt-auto space-y-3">
              <div className="glass p-3 rounded-xl">
                ...
                <div className="text-xs">
                  <div className="flex justify-between"><span className="text-slate-400">WebSocket</span><span className="text-emerald-300">Connected</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">AI Engine</span><span className="text-emerald-300">Online</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">Twilio</span><span className="text-emerald-300">Armed</span></div>
                </div>
              </div>
              <button
                onClick={() => { logout(); navigate("/"); }}
                className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm text-slate-400 hover:text-white hover:bg-white/5 border border-white/5"
              >
        ...
        <div className="flex-1 min-w-0">
          {/* Topbar */}
          <header className="sticky top-0 z-30 backdrop-blur-xl bg-[#05060f]/70 border-b border-white/5">
                ...
                <div className="overflow-hidden flex-1">
                  <div className="marquee-track text-slate-400">
                    {[
                      ...
                      <span key={i} className="text-slate-400">{m}</span>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <AnimatePresence>
          {notifications.map((n) => (
            <motion.div
              ...
              className={`glass-strong p-3 rounded-xl flex items-start gap-3 ${
                n.kind === "critical" ? "glow-pink" : n.kind === "warn" ? "glow-amber" : "glow-cyan"
              }`}
            >
              <div className={`w-8 h-8 rounded-lg grid place-items-center shrink-0 ${
                n.kind === "critical" ? "bg-pink-500/20 text-pink-300"
